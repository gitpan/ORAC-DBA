...
begin 
  ...
  loop 
    ...
    if delete_object = 2 then 
      break_pos := instr(upper(lv_text), 'PACKAGE'); 
      if break_pos != 0 then 
        delete_object := 1; 
        if length(lv_text) < break_pos + 8 then 
          goto fetchnext; 
        end if; 
        lv_text := substr(lv_text, break_pos + 7); 
      end if; 
    end if; 
    if delete_object = 1 then 
      break_pos := instr(upper(lv_text), 'BODY'); 
      if break_pos != 0 then 
        delete_object := 0; 
        if length(lv_text) < break_pos + 5 then 
          goto fetchnext; 
        end if; 
        lv_text := substr(lv_text, break_pos + 4); 
      end if; 
    end if; 
