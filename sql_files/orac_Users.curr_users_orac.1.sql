select s.username, osuser, sid, 
       decode(status,'ACTIVE','Act',
                     'INACTIVE','Inact','KILLED','Kill',status) status, 
       decode(type,'BACKGROUND','Back',
                   'USER','User',type) type, p.spid process, 
       s.terminal, decode(lockwait,'','','Y') lockwait, 
       decode(command,0,'',1,
                      'Create Table',2,'Insert',3,'Select',4,'Create Clust', 
       ...
       45,'Rollback',46,'Savepoint',47,'PL/SQL',to_char(command)) command 
