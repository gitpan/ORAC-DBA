declare
 ...
 cursor col_cursor is select
    ...
    where grantor != 'SYS'
    order by 2,3,4,5,1;
  cursor sys_cursor is select
    grantee,
    privilege,
    decode(admin_option,'YES',' WITH ADMIN OPTION;',';')
    from sys.dba_sys_privs
    order by 1,2;
  cursor role_cursor is select
    grantee,
    granted_role,
    decode(admin_option,'YES',' WITH ADMIN OPTION;',';')
...
begin
      ...
      if lv_owner != prev_own then
        if prev_own != '@' then
          ...
          if user_password != '<password>' then
            a_lin := wri(a_lin,'alter user '||lower(prev_own)||' identified by'||' values '||chr(39)||
                         user_password||chr(39)||';',1);
          else
  ...
  if prev_own != '@' then
    a_lin := wri(a_lin,'connect system/xyzzy',1);
    if user_password != '<password>' then
      a_lin := wri(a_lin,'alter user '||lower(prev_own)||' identified by values '||chr(39)||user_password||
                   chr(39)||';',1);
    else
      a_lin := wri(a_lin,'alter user '||lower(prev_own)||' identified by <password>;',1);
    end if;
  ...
  open sys_cursor;
  loop
    fetch sys_cursor into
      lv_grantee,
      lv_privilege,
      lv_grantable;
    exit when sys_cursor%NOTFOUND;
    if prev_grantee = lv_grantee and prev_grantable = lv_grantable
    then
      a_lin := wri(a_lin,', '||lv_privilege,0);
    else
      if prev_grantee != '@' then
