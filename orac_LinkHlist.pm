...
# endorsed or warrantied by any company
#
...
sub link_orac {
   $local_top = $_[0];
   $local_dbh = $_[1];
   $this_title = "Orac Links SQL $v_db";
   $dialog = $local_top->DialogBox( -title => $this_title,
                                       -buttons => [ "Dismiss" ]);
   $menu_bar = $dialog->Frame(width => 100)->pack(side => 'top', anchor => 'w');
   ...
   my $v_command = orac_Utils::file_string('sql_files', 'orac_LinkHlist',
                                           'link_orac','1','sql');
...
sub add_link {
   ...
   while (@v_this_text = $sth->fetchrow) {
      my $link = "$owner" . ':' . "$v_this_text[0]";
      $hlist->add($link, -itemtype => 'imagetext',
                            -image    => $file_bitmap,
                            -text     => $link);
   }
   my $rc = $sth->finish;
...
sub process_link {
   $dialog->Busy;
   my $input = $_[0];
   my($owner, $link, $dummy) = split(/:/, $input);
   do_a_link($input);
   $dialog->Unbusy;
}
sub do_a_link {
   my $input = $_[0];
   my($owner, $link, $the_task_ahead, $dummy) = split(/:/, $input);
   ...
   my $local_dialog = $dialog->DialogBox( -title => 'Orac Dialog',
                                          -buttons => [ "Dismiss" ]);
   $local_dialog->add("Label", -text => "Links SQL for $owner.$link", 
