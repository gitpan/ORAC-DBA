...
loop
<<fetchnext>>
fetch src_cursor into l_on,l_nm,l_text;
...
a_lin := wri(a_lin,'create or replace package body ',0);
...
if delete_object = 2 then
break_pos := instr(upper(l_text),'PACKAGE');
if break_pos != 0 then
delete_object := 1;
if length(l_text) < break_pos + 8 then
goto fetchnext;
end if;
l_text := substr(l_text,break_pos + 7);
end if;
end if;
if delete_object = 1 then
break_pos := instr(upper(l_text),'BODY');
if break_pos != 0 then
delete_object := 0;
if length(l_text) < break_pos + 5 then
goto fetchnext;
end if;
l_text := substr(l_text,break_pos + 4);
end if;
end if;
