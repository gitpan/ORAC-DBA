...
declare
cursor s_c is
select owner,name,text from sys.dba_source where type = 'FUNCTION'
and owner = ?
and name = ?
...
loop
<<fetchnext>>
fetch s_c into l_on,l_nm,l_text;
...
break_pos := instr(upper(l_text),'FUNCTION');
if break_pos != 0 then
delete_object := 0;
if length(l_text) < break_pos + 9 then
goto fetchnext;
end if;
l_text := substr(l_text,break_pos + 8);
end if;
end if;
