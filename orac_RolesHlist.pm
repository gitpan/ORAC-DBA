...
sub role_orac {
   $local_top = $_[0];
   $local_dbh = $_[1];
   $this_title = "Orac Roles SQL $v_db";
   ...
   my $v_command = orac_Utils::file_string('sql_files', 'orac_RolesHlist',
                                           'role_orac', '1','sql');
...
sub process_role {
   $dialog->Busy;
   my $input = $_[0];
   my($role, $bit, $dummy) = split(/\./, $input);
   do_a_role($input);
   $dialog->Unbusy;
}
sub do_a_role {
   my $input = $_[0];
   my($role, $bit, $the_task_ahead, $dummy) = split(/\./, $input);

   my $v_command = orac_Utils::file_string('sql_files', 'orac_RolesHlist',
                                           'do_a_role','1','sql');
   $v_command =~ s/orac_insert_role/$role/g;
   ...
   my $j_counter = 0;
   my $full_list;
   print TEXT "rem ----- Please protect this " .
              "output if password displayed ----- \n\n";
