...
sub snap_orac {
   $local_top = $_[0];
   $local_dbh = $_[1];
   $this_title = "Orac Snapshots SQL $v_db";
   ...
   my $v_command = orac_Utils::file_string('sql_files', 'orac_SnapHlist',
                                           'snap_orac', '1','sql');
...
sub add_snap {
   ...
   while (@v_this_text = $sth->fetchrow) {
      my $snap = "$owner" . '.' . "$v_this_text[0]";
      $hlist->add($snap, -itemtype => 'imagetext',
                            -image    => $file_bitmap,
                            -text     => $snap);
   }
   my $rc = $sth->finish;
...
sub process_snap {
   $dialog->Busy;
   my $input = $_[0];
   my($owner, $snap, $dummy) = split(/\./, $input);
   do_a_snap($input);
   $dialog->Unbusy;
}
sub do_a_snap {
   my $input = $_[0];
   my($owner, $snap, $the_task_ahead, $dummy) = split(/\./, $input);

   my $v_command = orac_Utils::file_string('sql_files', 'orac_SnapHlist',
                                           'do_a_snap','1','sql');
   $v_command =~ s/orac_insert_owner/$owner/g;
   $v_command =~ s/orac_insert_snap/$snap/g;
   ...
   $local_dialog->add("Label", 
                      -text => "Snapshots SQL for $owner.$snap", 
