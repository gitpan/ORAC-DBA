...
sub func_orac {
   $local_top = $_[0];
   $dbh = $_[1];
   $this_title = "Orac Functional SQL $v_db";
...
sub add_func {
   ...
   while (@v_this_text = $sth->fetchrow) {
      my $function = "$owner" . '.' . "$v_this_text[0]";
      $hlist->add($function, -itemtype => 'imagetext',
                              -image    => $file_bitmap,
                              -text     => $function);
   }
   my $rc = $sth->finish;
...
sub process_func {
   $top->Busy;
   my $input = $_[0];
   my($owner, $func, $dummy) = split(/\./, $input);
   do_a_func($input);
   $top->Unbusy;
}
sub do_a_func {
   my $input = $_[0];
   my($owner, $func, $the_task_ahead, $dummy) = split(/\./, $input);

   my $v_command = orac_Utils::file_string('sql_files', 'orac_FuncHlist',
                                           'do_a_func','1','sql');
   $v_command =~ s/orac_insert_owner/$owner/g;
   $v_command =~ s/orac_insert_func/$func/g;
   ...
   my $dialog = $top->DialogBox( -title => 'Orac Dialog', 
                                 -buttons => [ "Dismiss" ]);
   $dialog->add("Label", 
                -text => "Functional SQL for $owner.$func", 
