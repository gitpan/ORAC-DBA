...
sub seq_orac {
   $local_top = $_[0];
   $local_dbh = $_[1];
   $this_title = "Orac Sequence SQL $v_db";
   $dialog = 
   ...
   my $v_command = orac_Utils::file_string('sql_files', 'orac_SeqHlist',
                                           'seq_orac', '1','sql');
...
sub add_seq {
   ...
   while (@v_this_text = $sth->fetchrow) {
      my $seq = "$owner" . '.' . "$v_this_text[0]";
      $hlist->add($seq, -itemtype => 'imagetext',
                        -image    => $file_bitmap,
                        -text     => $seq);
   }
   my $rc = $sth->finish;
...
sub process_seq {
   $dialog->Busy;
   my $input = $_[0];
   my($owner, $seq, $dummy) = split(/\./, $input);
   do_a_seq($input);
   $dialog->Unbusy;
}
sub do_a_seq {
   my $input = $_[0];
   my($owner, $seq, $the_task_ahead, $dummy) = split(/\./, $input);

   my $v_command = orac_Utils::file_string('sql_files', 'orac_SeqHlist',
                                           'do_a_seq','1','sql');
   $v_command =~ s/orac_insert_owner/$owner/g;
   $v_command =~ s/orac_insert_seq/$seq/g;
   ...
   $local_dialog->add("Label", 
                      -text => "Sequence SQL for $owner.$seq", 
